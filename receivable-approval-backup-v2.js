/* BIG BROTHER — Receivable Request Approval Google Sheets Backup V2.1
...
(function(){
  ...
  function parsePayload(v){if(v&&typeof v==='object')return v;if(typeof v!=='string'||!v.trim())return{};try{return JSON.parse(v)}catch(_){return{}}}
  ...
  async function send(p){await fetch(ENDPOINT,{method:'POST',mode:'no-cors',cache:'no-store',keepalive:true,headers:{'Content-Type':'text/plain;charset=UTF-8'},body:JSON.stringify(p)});return true}
  ...
  function rememberList(data){(Array.isArray(data?.receivables)?data.receivables:[]).forEach(r=>{const no=clean(r?.invoiceNo);if(no)invoiceCache.set(no,r)})}
  function rememberRequest(data){const r=data?.request,id=clean(r?.requestId);if(id)requestCache.set(id,r);return r||null}
  ...
  async function backupRequest(request,payload,result){
    ...
    for(const a of allocations){
      const invoiceNo=clean(a?.invoiceNo),amount=num(a?.requestedAmount);
      ...
      const rate=num(payload?.exchangeRate||result?.exchangeRate||a?.exchangeRate||invoice?.exchangeRate);
      ...
      sendOrQueue({
        backupType:'RECEIVABLE_PAYMENT',invoiceNo,customerName:clean(request?.customer||invoice?.customer),paymentAmountUSD:amountUSD,paymentDate,
        paymentMethod:clean(payload?.paymentMethod||result?.paymentMethod||request?.paymentMethod),transactionId:clean(payload?.transactionId||result?.transactionId||request?.transactionId),
        salesman:clean(request?.salesmanName||request?.requestedBy||invoice?.salesmanName||invoice?.salesperson),location:clean(a?.locationCode||invoice?.locationCode),note:clean(request?.note),
        supabasePaymentId:paymentId?paymentId+':'+invoiceNo:'',supabaseInvoiceId:clean(a?.invoiceId||invoice?.invoiceId),createdBy:userEmail(),paymentCurrency:currency,
        originalPaymentAmount:amount,exchangeRate:rate,backupKey:rowKey
      }).catch(()=>{});
    }
    return true;
  }
  ...
  async function handle(params={},result={}){
    ...
    if(action==='arClearRequest'&&result?.success){
      const payload=parsePayload(params?.requestData),req=requestCache.get(clean(payload?.requestId))||null;
  ...
  function wrapAdminAdapter(){
    ...
    const raw=a.apiPost;
    const wrapped=async function(params={}){const result=await raw.call(a,params);handle(params,result).catch(e=>console.warn('BIG BROTHER Admin Work backup:',e));return result};
    wrapped.__bbApprovalBackupV2=true;a.apiPost=wrapped;return true;
  }

  function loadCurrencyUi(){
    if(window.BBReceivableApprovalCurrencyV1||document.querySelector('script[data-bb-approval-currency="1"]'))return;
    const script=document.createElement('script');script.src='receivable-approval-currency-v1.js?v=20260916-2';script.async=false;script.dataset.bbApprovalCurrency='1';
    (document.head||document.documentElement).appendChild(script);
  }

  if(!wrapAdminAdapter()){let tries=0;const timer=setInterval(()=>{tries++;if(wrapAdminAdapter()||tries>=50)clearInterval(timer)},100)}
  retryQueue().catch(()=>{});setTimeout(loadCurrencyUi,0);
